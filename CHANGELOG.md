...
### Fixed
- **角色参考图重复生成与乱选** — 同一资产的生成提交和后台任务执行增加 single-flight 保护；执行异常会持久化失败状态。角色在分镜中只使用当前选中的参考图，历史变体继续保留在图库供用户检查。
- **角色参考图默认批量** — 资产生成默认一次生成一张，用户仍可明确选择更大的批量。
...
### Fixed
- **资产库封面更新** — 封面改为独立持久化的变体 ID；生成只加入候选图，不会覆盖用户选定的封面。成功更新以紧凑响应刷新当前卡片，不再重新读取整个项目或资产库。
- **资产引用索引语义** — `cover_variant_id` 与生成容器的 `selected_variant_id` 分开，设置封面不会改变生成引用默认选择。
- **资产任务恢复与引用刷新** — 终态恢复和项目资产变更后索引刷新有针对性测试，降低刷新后任务长期停留在 generating 或引用过期的风险。
