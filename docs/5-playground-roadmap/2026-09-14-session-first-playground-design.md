...
## Phase 1.2 — Agent 式创作交互

### 目标

将固定左侧表单改为单列 Agent 会话工作台。默认界面只保留当前 Session 消息流与底部统一输入框；输出模式、参考素材、模型和参数作为输入框工具按需展开，以释放结果展示空间。
...
│  user request                                                 │
│  generation status → result cards                             │
│  user refinement                                              │
│  generation status → result cards                             │
│                                                              │
│  ┌──────────────── Agent Composer ─────────────────────────┐  │
│  │ optional expanded tool panel                            │  │
│  │ prompt / negative prompt                               │  │
│  │ [mode] [media] [model] [templates] [history]      [↑] │  │
│  └─────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────┘
```

- 不保留固定 Compose 侧栏；消息流获得主区域全部可用宽度。
- 底部 Composer 是唯一主输入面，草稿仍按当前 Session 自动保存。
- 模式、参考素材、SKU 和参数使用互斥 Popover 菜单渐进展开，不改变 Composer 高度；打开一个工具时关闭其他工具，点击外部或按 Esc 关闭。
- 工具按钮显示当前上下文摘要，例如模式、模型名称和参考素材数量，避免面板折叠后状态不可见。
- 主工具栏使用产品语义而非协议缩写：第一项显示“图片 / 视频”，T2I、I2V、R2V 等只在生成方式面板中显示。
- SKU 模型选择与参数设置拆成独立工具。顺序为输出类型、SKU、参数、参考素材；参数摘要优先展示尺寸/分辨率、Ratio、时长、Seed 和生成数量。
- Seed 与生成数量属于常用生成参数，直接显示在参数主面板，不隐藏在高级设置中；提示词扩展和水印继续归入高级设置。
- 模板与全部历史继续使用现有抽屉，不复制新的数据或持久化结构。
- 提交生成后，任务作为同一轮消息中的状态卡出现；轮询得到的新状态和结果原位更新，不宣称或模拟后端不存在的 token streaming。
- 新轮次和任务状态变化后，消息流自动滚动到底部；用户主动向上阅读历史时不应被持续强制拉回。
- 桌面与移动端使用同一交互模型，工具栏允许换行但不得产生横向滚动。

### Phase 1.2 Success Criteria

1. 默认状态下创作输入只占底部 Composer，不再占用固定 350–380px 侧栏。
2. 六种既有生成模式、参考素材、模型和参数均可从 Composer 恢复访问。
3. 当前模式、模型和素材数量在折叠状态下仍可识别。
4. 生成中、失败和完成结果继续在对应消息轮次中原位展示。
5. Session、模板、全部历史、草稿自动保存和历史恢复能力不回归。
