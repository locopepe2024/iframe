...
UI: ARIA modal portal (not native showModal: engine menus portal to body), inert background, focus containment/restoration, Escape/close dirty confirmation, busy guard, asynchronous save errors retain edits; translated labels; lazy-loaded engine. Start on Rotate so opening an image does not introduce a crop; save directly to the host without the redundant engine save-as modal. Source switching creates a new editor instance. Successful saves append only if the active session still matches the opening session; copies remain in history regardless.
