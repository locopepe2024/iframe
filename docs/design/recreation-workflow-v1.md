...
日期：2026-09-17。状态：复刻链路已在当前 iframe 分支实现；线上 Provider 与视觉质量仍待验收。分支：fix/recreation-gpt-image-25。
...
实测只检测到两个切点，9.083333 秒仍需人工确认，不据单个样本降低阈值。
分析完成时登记派生图片并保存 SHA-256、原片 ID、分析 ID 和 PTS；同一文件
被采样和证据复用时保持一个媒体 ID。手动证据作为分析的增量信息保存，不改变
时间线 revision，确认/刷新后继续可用。证据接口复用已保存结果。

旧项目通过 `POST /recreation/projects/{id}/reindex` 补登记现有分析文件；重复调用
保留媒体 ID，不改已确认时间线。部署后版本管理员可为验收项目执行该操作。
不能恢复此前未记录的手动证据文件，需要再次加载该切点一次以建立持久关联。
检索过滤与 limit/offset 在 SQLite 执行，以 created_at/media_id 固定排序；offset
分页不保证并发新增时的快照一致性。媒体名包含原片名、帧角色及 PTS。
本修复不宣称已有通用资产管理页面、标签搜索或真实 Provider 视觉质量验收；当前 H3 生成/装配任务闭环已实现，仍需线上验收。
...
- 原始首切片曾限定不提交 H3；当前分支已在后续切片实现替换素材、H3 generation task 和本地最终装配，但仍不代表已部署生产或完成视觉验收。变更通过 PR 交版本管理员审核。

- 历史首切片：原片归属注册 → 真实PTS探测 → 候选切点与证据帧输出；该部分已完成并有合成/桌面样本测试。
- 当前后续工作：受控真实 Provider 端到端验收、运动参考输入契约和视觉质量记录。
