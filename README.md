...
iFrame 是一个 **AI 原生的短漫剧 & 视频创作平台**。它将创意文本转化为可发布的动态视频，提供从剧本分析到成片导出的完整创作链路，同时支持独立的图像/视频生成能力。

iFrame 目前包含两个核心模块：

| 模块 | 定位 |
|------|------|
| **iFrame Studio** | Pipeline-first 漫剧/视频生产（剧本→分镜→资产→视频→合成→导出） |
| **iFrame Playground** | 独立图像/视频生成工具台（无需剧本上下文，即开即用） |
...
| Before | After |
|:---:|:---:|
| <img src="docs/images/iFrame Studio Banner.jpeg" alt="旧版 Banner" width="100%" /> | <img src="docs/images/iFrame-Studio-Banner-cybr.png" alt="新版 Banner" width="100%" /> |
...
iFrame 采用 **本地优先** 的架构，最简配置只需一个 API Key。
...
所有配置可通过以下方式设置：
- **开发模式**: 项目根目录 `.env` 文件
- **应用内设置**: Settings 页面（保存到 `~/.iframe/config.json`）
...
- **Bug 反馈**: [GitHub Issues](https://github.com/locopepe2024/iframe/issues)
- **功能建议**: [GitHub Discussions](https://github.com/locopepe2024/iframe/discussions)
